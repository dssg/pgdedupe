...
def merge(mapping_table, entries_table, exact_columns, con):
    edges = pd.read_sql("""
    with subset as (
        SELECT _unique_id, canon_id, {columns}
        FROM {entries} LEFT JOIN {mapping} using (_unique_id)
    )

    SELECT t1.canon_id id1, id2 from
    subset t1 JOIN
    (SELECT min(canon_id) id2, {columns} from subset group by {columns} having count(*) > 1) t
    using ({columns})
    where t1.canon_id > id2
    group by 1,2
    """.format(columns=', '.join(exact_columns), entries = entries_table, mapping = mapping_table), con)
